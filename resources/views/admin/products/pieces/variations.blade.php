            ...
            <div class="option-value-rowid attribute-dlt">
                ...
                <div class="collapse collaspe-sec txt-enable" id="{{ $unid }}">
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                <a href="javascript:void(0)" data-name="meta_image[{{ $idx }}]"
                                   data-val="single" class="btn-cancels ml-20p {{ $image ? 'variation-image-remove' : 'variation-image has-media-manager' }}">
                                    {{ $image ? __('Remove') : __('Upload Photo') }}
                                </a>
                            </div>
                        </div>
                    ...
                    <div class="d-flx d-flx-n align-items-center mx-7p border-bottom mt-25p">
                        ...
                        <div class="mr-35p mt-10-res {{ isActive('SaaS') ? 'd-none' : '' }}">
                        ...
                        <div class="mr-35p mt-10-res">
                            <div
                                ...
                                href="#{{ $stockId = 'stock' . uniqid() }}" type="checkbox"
                                       ...
                                       class="{{ $variation->manage_stocks == '1' ? 'collapse show' : 'collapsed collapse' }} variation-manage-stock"
                                       value="1">
                                <label for="{{ $stockId }}"
                    ...
                    <div id="{{ $stockId }}"
                        ...
                        <div class="mt-3">
                            <div class="checkbox checkbox-primary d-inline w-space">
                                <input type="hidden" name="meta_hide_stock[{{ $idx }}]" value="0">
                                <input type="checkbox" id="is_hide_stock[{{ $idx }}]" name="meta_hide_stock[{{ $idx }}]" value="1"
                                    {{ $variation->hide_stock == 1 ? 'checked' : '' }}>
                                <label for="is_hide_stock[{{ $idx }}]" class="crs sp-title d-flx-n">{{ __('Hide Stock') }}</label>
                            </div>
                        </div>
                    </div>
                    <div class="row form-group px-7 variation-stock-status {{ $variation->manage_stocks == '1' ? 'd-none' : '' }}" data-stock-id="{{ $stockId }}">
                        <div class="col-md-12">
                            <label for="status"
                                class="sp-title text-14 control-label sl_common_bx rtl:text-end">{{ __('Stock Status') }}</label>
                        </div>
                        <div class="col-md-6">
                            <select class="form-control select2" name="meta_stock_status[{{ $idx }}]" data-minimum-results-for-search="Infinity">
                                <option value="In Stock"
                                    {{ $variation->meta_stock_status == 'In Stock' ? 'selected' : '' }}>
                                    {{ __('In Stock') }}</option>
                                <option value="Out Of Stock"
                                    {{ $variation->meta_stock_status == 'Out Of Stock' ? 'selected' : '' }}>
                                    {{ __('Out Of Stock') }}</option>
                            </select>
                        </div>
                    </div>
                    @endifSettings
                    <div class="row form-group px-7">
                        <div class="{{ isActive('SaaS') ? 'display-none' : '' }} ">
                            <div class="checkbox checkbox-primary d-inline w-space">
                                <input type="hidden" name="meta_individual_sale[{{ $idx }}]" value="0">
                                <input type="checkbox" id="is_individual_sale[{{ $idx }}]" name="meta_individual_sale[{{ $idx }}]" value="1"
                                    {{ $variation->individual_sale == 1 ? 'checked' : '' }}>
                                <label for="is_individual_sale[{{ $idx }}]" class="crs sp-title d-flx-n">{{ __('Sold Individually (Enable this to only allow one of this Product to be bought in a single order)') }}</label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="{{ $virId }}"
                        ...
                        <div class="col-md-6">
                            <div class="d-flex align-items-center">
                                <label for="Weight" class="sp-title control-label">{{ __('Unit') }}</label>
                            </div>
                            <select class="form-control select2 sl_common_bx" name="meta_unit[{{ $idx }}]">
                                @foreach ($units as $unit)
                                    <option @selected(
                                            (isset($variation) && $variation?->unit == $unit->abbr) ||
                                            (!isset($variation) && $unit->default == 'Yes')
                                        ) 
                                        value="{{ $unit->abbr }}">{{ $unit->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="form-group row px-7 mt-20p {{ isActive('SaaS') ? 'display-none' : '' }}">
                        <div class="col-md-12">
                            ...
                            <textarea class="form-control" placeholder="{{ __('Variable Description') }}" name="variable_description[{{ $idx }}]"
                                      rows="2">{{ $variation->getTranslated('description', request()->input('lang', config('app.locale'))) }}</textarea>
                        </div>
                    </div>
                    @php
