    ...
    <div class="col-sm-12" id="user-add-container">
        <div class="card">
            <div class="card-header">
                <h5> <a href="{{ route('customer.addresses.index', ['customer' => $address->customer_id, 'vendor' => $address->vendor_id] ) }}">{{ __('Customer Billing & Shipping') }} </a>
                    >> {{ __('Update :x', ['x' => __('Billing & Shipping')]) }}</h5>
            </div>
            <div class="card-block table-border-style">
                <ul class="nav nav-pills" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link h-lightblue ltr:pe-2 rtl:ps-2" href="{{ route('customers.edit', $address->customer_id) }}" role="tab" aria-controls="mcap-default" aria-selected="true">{{ __('Profile') }}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link h-lightblue ltr:pe-2 rtl:ps-2 active" href="{{ route('customer.addresses.index', ['customer' => $address->customer_id]) }}" role="tab" aria-controls="mcap-default" aria-selected="true">{{ __('Billing & Shipping') }}</a>
                    </li>
                </ul>
                <div class="row form-tabs">
                    <form action="{{ route('customer.addresses.update', $address->id) }}" method="post" id="userAdd"
