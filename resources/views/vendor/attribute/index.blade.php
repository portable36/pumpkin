        ...
        <div class="card">
            <div class="card-header bb-none pb-0">
                ...
                <div class="card-header-right my-2 mx-md-0 mx-sm-4">
                    @hasPermission('App\Http\Controllers\Vendor\AttributeController@create')
                        <x-backend.button.add-new href="{{ route('vendor.attribute.create') }}" />
                    @endhasPermission
...
@section('js')
    <script type="text/javascript">
        'use strict';
        var pdf = "{{ auth()->user()?->hasPermission('App\Http\Controllers\Vendor\AttributeController@pdf') ? '1' : '0' }}";
        var csv = "{{ auth()->user()?->hasPermission('App\Http\Controllers\Vendor\AttributeController@csv') ? '1' : '0' }}";
    </script>
