...
@section('page_title', __('Update :x', ['x' => __('Customer Address')]))
@section('css')
    ...
    <div class="col-sm-12" id="user-add-container">
        <div class="card">
            <div class="card-header">
                <h5> <a href="{{ route('vendor.customer.addresses', ['customer' => $address->customer_id] ) }}">{{ __('Customer Billing & Shipping') }} </a>
                    >> {{ __('Update :x', ['x' => __('Billing & Shipping')]) }}</h5>
            </div>
            <div class="card-block table-border-style">
                <ul class="nav nav-pills" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link h-lightblue ltr:pe-2 rtl:ps-2" href="{{ route('vendor.customer.edit', $address->customer_id) }}" role="tab" aria-controls="mcap-default" aria-selected="true">{{ __('Profile') }}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link h-lightblue ltr:pe-2 rtl:ps-2 active" href="{{ route('vendor.customer.addresses', ['customer' => $address->customer_id]) }}" role="tab" aria-controls="mcap-default" aria-selected="true">{{ __('Billing & Shipping') }}</a>
                    </li>
                </ul>
                <div class="row form-tabs">
                    <form action="{{ route('vendor.customer.addresses.update', $address->id) }}" method="post" id="userAdd"
                        ...
                        <ul class="nav nav-tabs" id="myTab" role="tablist">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active text-uppercase font-bold" id="home-tab" data-bs-toggle="tab" href="#home"
                                    ...
                                    aria-selected="true">{{ __(':x Information', ['x' => __('Address')]) }}</a>
                            </li>
                        ...
                        <div class="col-sm-12 tab-content form-edit-con" id="myTabContent">
                            <div class="tab-pane fade show active form-con" id="home" role="tabpanel"
                                ...
                                @include('common.customer-address.edit')
                                <div class="d-flex mt-30p">
                                    <a href="{{ route('vendor.customer') }}"
