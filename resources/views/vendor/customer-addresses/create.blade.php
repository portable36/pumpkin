    ...
    <div class="col-sm-12" id="user-add-container">
        <div class="card">
            <div class="card-header">
                <h5> <a href="{{ route('vendor.customer.addresses', ['customer' => $customerId] ) }}">{{ __('Customer Billing & Shipping') }} </a>
                    >> {{ __('Create :x', ['x' => __('Billing & Shipping')]) }}</h5>
            </div>
            <div class="card-block table-border-style">
                <ul class="nav nav-pills" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link h-lightblue ltr:pe-2 rtl:ps-2" href="{{ route('vendor.customer.edit', $customerId) }}" role="tab" aria-controls="mcap-default" aria-selected="true">{{ __('Profile') }}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link h-lightblue ltr:pe-2 rtl:ps-2 active" href="{{ route('vendor.customer.addresses', ['customer' => $customerId]) }}" role="tab" aria-controls="mcap-default" aria-selected="true">{{ __('Billing & Shipping') }}</a>
                    </li>
                </ul>
                <div class="row form-tabs">
                    <form action="{{ route('vendor.customer.addresses.store') }}" method="post" id="userAdd"
