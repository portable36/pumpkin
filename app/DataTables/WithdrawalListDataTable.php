...
use App\Filters\WithdrawalFilter;
use App\Models\{
    Transaction
};
...
class WithdrawalListDataTable extends DataTable
{
    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($transactions) {
                ...
                $str = '';
                if (auth()->user()?->hasPermission('App\Http\Controllers\WithdrawalController@edit')) {
                    $str .= $edit;
                }
    ...
    */
    ...
    {
        $transactions = Transaction::select('transactions.id', 'user_id', 'currency_id', 'transactions.status', 'withdrawal_method_id', 'amount', 'charge_amount', 'commission_amount', 'discount_amount', 'total_amount')->where('transaction_type', 'Withdrawal')->with(['user:id,name', 'currency:id,name', 'withdrawalMethod:id,method_name'])->filter(WithdrawalFilter::class);
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => '', 'width' => '5%',
                'visible' => auth()->user()?->hasAnyPermission(['App\Http\Controllers\WithdrawalController@edit']),
