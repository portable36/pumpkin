...
class BrandListDataTable extends DataTable
{
    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($brands) {
                ...
                $str = '';
                if (auth()->user()?->hasPermission('App\Http\Controllers\BrandController@edit')) {
                    $str .= $edit;
                }
                if (auth()->user()?->hasPermission('App\Http\Controllers\BrandController@destroy')) {
    ...
    */
    ...
    {
        $brands = Brand::where('is_global', 1)->withCount('product');
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => '', 'width' => '10%',
                'visible' => auth()->user()?->hasAnyPermission(['App\Http\Controllers\BrandController@edit', 'App\Http\Controllers\BrandController@destroy']),
