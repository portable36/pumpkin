...
class OrderDataTable extends DataTable
{
    ...
    public function ajax(): JsonResponse
    {
        ...
        $dt = datatables()
            ...
            ->editColumn('customer', function ($orders) {
                $person = $orders->user ?? $orders->customer;

                if ($person?->id) {
                    $route = $orders->user ? route('users.edit', ['id' => $person->id])
                                            : route('customers.edit', ['customer' => $person->id]);

                    return '<a target="_blank" href="' . $route . '">'
                        . wrapIt(! empty($person->name) ? $person->name : $person->phone, 10, ['columns' => 2])
                        . '</a>';
                }
                ...
                $orderDetails = \App\Models\OrderDetail::where('order_id', $orders->id)->with('vendor')->get();
                $vendorLinks = [];
                $seenVendorIds = [];

                foreach ($orderDetails as $detail) {
                    if ($detail->vendor && $detail->vendor->id && !in_array($detail->vendor->id, $seenVendorIds)) {
                        $seenVendorIds[] = $detail->vendor->id;
                        $vendorName = $detail->vendor->name;
                        $vendorLinks[] = '<a href="' . route('vendors.edit', ['id' => $detail->vendor->id]) . '">' . wrapIt($vendorName, 10, ['columns' => 2]) . '</a>';
                    }
                }

                return !empty($vendorLinks) ? implode(', ', $vendorLinks) : '';
            });
        ...
        $dt->addColumn('action', function ($orders) {
            ...
            $str = '';
            if (in_array($orders->channel, ['invoice', 'pos']) && auth()->user()?->hasPermission('App\Http\Controllers\AdminOrderController@edit') && $orders->total > $orders->amount_received) {
                $str .= '<a title="' . __('Edit') . '" href="' . route('order.edit', ['id' => $orders->id]) . '" class="action-icon view-order" data-id=' . $orders->id . ' ><i class="feather icon-edit"></i></a>';
            }
            ...
            ->rawColumns(['customer', 'total', 'status', 'order_date', 'reference', 'action', 'payment_status', 'vendor']);
    ...
    */
    ...
    {
        $orders = $orders = Order::select('orders.id', 'user_id', 'customer_id', 'reference', 'order_date', 'currency_id', 'other_discount_amount', 'other_discount_type', 'shipping_charge', 'tax_charge', 'total', 'amount_received', 'paid', 'order_status_id', 'payment_status', 'orders.channel')
            ->with(['customFieldValues', 'orderDetails:id,product_id,order_id,vendor_id,shop_id,price,quantity,discount_amount,discount_type,order_status_id', 'orderStatus:id,slug,name', 'user:id,name', 'customer:id,name,phone'])->filter();
    ...
    */
    public function html()
    {
        $builder = $this->builder()
            ...
            ->addColumn(['data' => 'channel', 'name' => 'channel', 'title' => __('Channel'), 'className' => 'align-middle']);
