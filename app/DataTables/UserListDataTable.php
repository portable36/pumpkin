    ...
    public function ajax(): JsonResponse
    {
        ...
        $dt->editColumn('created_at', function ($users) {
            ...
            ->addColumn('action', function ($users) {
                ...
                $url = optional($users->role())->slug === 'delivery-man' ? route('admin.delivery.carrier.edit', optional($users->deliveryMan)->id) : route('users.edit', ['id' => $users->id]);

                $edit = '<a data-bs-toggle="tooltip" title="' . __('Edit') . '" href="' . $url . '" class="action-icon"><i class="feather icon-edit-1"></i></a>';
