...
class ProductExportService extends ExportService
{
    ...
    public function __construct(Request $request)
    {
        ...
        if (isActive('SaaS')) {
            unset($this->productType['Grouped Product']);
            unset($this->productType['External/Affiliate Product']);
        }
    }
    ...
    public function getColumns(): void
    {
        ...
        if (isActive('SaaS')) {
            $products->whereNotIn('type', ['External/Affiliate Product', 'Grouped Product']);
        }
        ...
        if (isset(auth()->user()->role()->slug) && isset(auth()->user()->vendor()->vendor_id) && auth()->user()->role()->type == 'vendor') {
        ...
        // Initialize base columns if no products found to ensure headers are always exported
    ...
    /**
     * Initialize base columns for CSV export
     * This ensures headers are exported even when no products are found
     */
    protected function initializeBaseColumns(): void
    {
        $exportedColumn = isset($this->request->exported_column) ? $this->request->exported_column : [];
        ...
        // Remove SaaS-specific columns if SaaS is active
        if (isActive('SaaS')) {
            $baseColumns = array_diff($baseColumns, [
                'Description',
                'Short description',
                'Is featured?',
                'Weight (lbs)',
        ...
        // If specific columns are requested, only include those (excluding dynamic ones)
        if (count($exportedColumn) > 0) {
            $this->csvColumn = array_filter($exportedColumn, function ($column) {
                return ! in_array($column, ['Attributes', 'Downloads', 'B2B']);
            });

            // Add B2B base column if B2B is requested and active
            if (isActive('B2B') && in_array('B2B', $exportedColumn)) {
                $this->csvColumn[] = 'Is B2B?';
            }
        } else {
            // Include all base columns
            $this->csvColumn = $baseColumns;

            // Add B2B base column if B2B is active
            if (isActive('B2B')) {
                $this->csvColumn[] = 'Is B2B?';
            }
        }
    ...
    public function loadProductData($product): array
    {
        ...
        $attributeRetriveAdditionalValue = [];
        $downloadData = [];
        ...
        if (count($exportedColumn) == 0 || in_array('Attributes', $exportedColumn)) {
            ...
            foreach ($product->getProductAttributes() as $attribute) {
                ...
                if (isset($attribute['additional_value']) && is_array($attribute['additional_value'])) {
                    foreach ($attribute['additional_value'] as $additionalValue) {
                        if (! empty($additionalValue)) {
                            $attributeRetriveAdditionalValue[] = $additionalValue;
                        }
                    }
                }
                ...
                $attributeData['Attribute' . ' ' . $index . ' ' . 'type'] = isset($attribute['type']) ? $attribute['type'] : 'dropdown';
                $attributeData['Attribute' . ' ' . $index . ' ' . 'additional_value'] = count($attributeRetriveAdditionalValue) > 0 ? implode(',', $attributeRetriveAdditionalValue) : null;
        ...
        if (count($exportedColumn) == 0 || in_array('Downloads', $exportedColumn)) {
            foreach ($product->getDownloadables() as $download) {
                $downloadData['Download' . ' ' . $downloadIndex . ' ' . 'name'] = isset($download['name']) ? $download['name'] : '';
                $downloadData['Download' . ' ' . $downloadIndex . ' ' . 'URL'] = isset($download['url']) ? $download['url'] : '';
                $downloadIndex++;
            }
        }
        ...
        $data = [
            ...
            'Shipping class' => $product->meta_shipping_id == 'Select One' ? null : $product->meta_shipping_id,
        ...
        if (isActive('SaaS')) {
            foreach (['Description', 'Short description', 'Is featured?', 'Weight (lbs)', 'Height (in)', 'Length (in)', 'Width (in)', 'Button text', 'External URL', 'Grouped products', 'Upsells', 'Cross-sells', 'Allow customer reviews?', 'Purchase note', 'Download limit', 'Download expiry days', 'Sold individually?', 'Downloads'] as $key) {
                unset($data[$key]);
            }
        }
