...
abstract class ImportService
{
    ...
    public function loadFile($fileInputName = 'csv')
    {
        ...
        $path = 'public' . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . $name;
    ...
    protected function processImportableData($columns, $callback)
    {
        ...
        foreach ($reader->getRow($this->getDelimiter()) as $key => $row) {
            ...
            if ($formattedData) {
                ...
                if (! isset($formattedData['parent_id']) || $formattedData['parent_id'] === '') {
                    $parentProduct++;
                }
            }

            if (isActive('Subscription') && $userType == 'vendor' && $parentProduct == $limit) {
                break;
            }
