...
class ProductImportService extends ImportService
{
    protected $fieldMap = [
        ...
        'attributeadditionalvalue' => 'attributeadditionalvalue',
    ...
    protected function storeSingleProduct($product)
    {
        try {
            return \DB::transaction(function () use ($product) {
                $totalStocks = $product['total_stocks'] ?? 0;
                ...
                $product->total_stocks = $totalStocks;
                $product->save();
    ...
    {
        $data['vendor_id'] = $data['vendor_id'] ?? auth()->user()->vendor()?->vendor_id;
    ...
    {
        $data['total_stocks'] = $data['total_stocks'] ?? 0;

        $data['manage_stocks'] = $data['manage_stocks'] ?? 0;
    ...
    private function getAttributesFromProduct($product)
    {
        ...
        foreach ($product['attribute'] as $index => $attribute) {
            ...
            $attribute->additional_values = array_map('trim', explode(',', $attribute->additional_values));

            if ($attribute->variation == 1) {
                $fetchAttribute =  Attribute::updateOrCreate(['name' => $attribute->title]);
                $attribute->value = $this->getAttributeValue($fetchAttribute->id, $attribute->value, $attribute->additional_values);
            ...
            $attribute = $this->checkAttributeValidity([
                ...
                'type' => $attribute->type,
                'additional_values' => $attribute->additional_values ?? '',
     ...
     * @return array
     */
    private function getAttributeValue($attribute, $values, $additionalValues = '')
    {
        $attributeValues = [];
        // Prepare attribute values for insertion
        foreach ($values as $key => $value) {
            $attributeValues[] = [
                'attribute_id' => $attribute,
                'value' => $value,
                'additional_values' => isset($additionalValues[$key]) ? $additionalValues[$key] : '',
                'order_by' => 0,
            ];
        }
        ...
        return function ($field) {
            ...
            $data = [
                ...
                __('Attributes') => [
                    ...
                    'attribute:type.' . $index => __('Attribute Type'),
                    'attribute:additional_values.' . $index => __('Attribute Additional Values'),
