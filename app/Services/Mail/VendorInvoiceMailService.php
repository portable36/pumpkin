...
class VendorInvoiceMailService extends TechVillageMail
{
    ...
    public function send($request)
    {
        ...
        $email = $this->getTemplate(preference('dflt_lang'), 'vendor-invoice');
        ...
        $customTax = $request->updatedCustomTaxFee($request, true, true, $vendorId);
        $customFee = $request->customFeeCalculations();
        ...
        $data = [
            ...
            '{custom_fee}' => formatCurrencyAmount($customFee['feeTotal']),
            '{grand_total}' => formatCurrencyAmount($subTotal + $taxCharge + $shippingCharge - $discount + $customTax + $customFee['feeTotal'] + $customFee['customTaxTotal']),
            ...
            '{tax_charge}' => formatCurrencyAmount($taxCharge + $customFee['customTaxTotal'] + $customTax),
