...
class ProductDataTabs
{
    ...
    public function tabs()
    {
        $tabs = [
            ...
            'Inventory' => [
                'tab' => '<li class="nav-item conditional-dom not-variable-dom ' . ($this->isTabDisplayNone('Inventory') ? 'd-none' : '') . '">
            ...
            'Shipping & Delivery' => [
                'tab' => '<li class="nav-item off_impact not-variable-dom conditional-dom not-grouped-dom not-external-dom ' .
    ...
    private function isTabDisplayNone($tabName)
    {
        switch ($tabName) {
            ...
            case 'Inventory':
                return ! is_null($this->product) && $this->product->isVariableProduct();
            default:
                return false;
        }
    }
