...
class AddToCartService
{
    ...
    public function add($request)
    {
        ...
        $add = Cart::add(
            [
                ...
                'error_type' => isset($response['error_type']) ? $response['error_type'] : null,
            ]
        );
    ...
    public function cartList()
    {
        ...
        foreach ($cartsValue as $key => $val) {
            ...
            if (empty($product) || $product->getStockStatus() == 'Out Of Stock') {
                Cart::destroy($key);
            ...
            $cartWithIndex[] = [
                ...
                'tax_rates' => $product->getTaxRates(),
                'error_type' => isset($val['error_type']) ? $val['error_type'] : null,
                'error_message' => isset($val['error_message']) ? $val['error_message'] : null,
            ];
        }
        ...
        if (in_array(ltrim(request()->route()->getPrefix(), '/'), ['api/user', 'api/pos'])) {
            return $data;
        }
    ...
    public function getTaxShipping($address = null, $from = null, $isSessionAble = false)
    {
        ...
        foreach ($cartsValue as $val) {
            ...
            $offerFlag = $product->offerCheck();
            $params = array_merge(
                ['qty' => $val['quantity'], 'cart_price' => $val['price']],
                request()->has('custom_price') ? ['totalPrice' => $val['price']] : []
            );
            if ($displayTaxTotal == 'itemized') {
                $itemizedTax = $offerFlag ? $product->priceWithTax('including tax', 'sale', false, false, true, $address, $shipCharge, $params) : $product->priceWithTax('including tax', 'regular', false, false, true, $address, $shipCharge, $params);
                $tax = $this->mergeProductizedTax($itemizedTax);
            } else {

                $tax += $offerFlag ? $product->priceWithTax('including tax', 'sale', false, true, false, $address, $shipCharge, $params) : $product->priceWithTax('including tax', 'regular', false, true, false, $address, $shipCharge, $params);
            }
        }
     ...
     * @return void
     */
    public function addPos($request)
    {
        $product = ProductAction::execute('getProductWithAttributeAndVariations', $request);
        $data = (new ProductDetailResource($product))->toArray(null);
        $variation = null;
        ...
        if ((! $product->availability() && $product->status == 'Published') || $product->status != 'Published' || $product->isExternalProduct() || $product->isGroupedProduct()) {
            $response['error_type'] = 'unavailable';
            $response['error_message'] = __('This product is currently unavailable.');
        }
        ...
        if ($isManageStock == 1 && $request->qty > $totalStock && $backOrder == 0 || $isManageStock == 1 && $this->existQty($totalStock, $request->qty, $product->isVariableProduct() ? $variation->id : $data['id']) && $backOrder == 0 || $isManageStock == 0 && $data['stock_status'] == 'Out Of Stock') {
            $response['error_type'] = 'stock';
            $response['error_message'] =  __('Stock is not available.');
        }
