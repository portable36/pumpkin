    ...
    protected function barcodeCollection()
    {
        $this->collection->each(function ($collection) {
            if ($this->option['barcode_type'] == 'QRCODE' || $this->option['barcode_type'] == 'PDF417' || $this->option['barcode_type'] == 'DATAMATRIX') {
                ...
                $barcode = (new DNS1D())->getBarcodePNG((string) $collection->{$this->option['link_to']}, $this->option['barcode_type'], $this->option['barcode_width'], $this->option['barcode_height'], json_decode($this->option['barcode_color'], true), $this->option['barcode_text'] == 1);
            }
