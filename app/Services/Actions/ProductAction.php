...
use App\Models\{Attribute, AttributeValue, Language, Product, ProductCategory, ProductCrossSale, ProductRelate, ProductTag, ProductMeta, ProductUpsale, Tag, Unit, Vendor};
...
class ProductAction
{
    ...
    protected function updatePermalink()
    {
        ...
        $this->product->setTranslation('slug', request()->input('lang', config('app.locale')), $pl);
    ...
    protected function updateBasicInfoWeb()
    {
        if ($this->request->data) {
            ...
            $data = $this->productOthersDataTranslate($data);
        ...
        return redirect()->route($this->getRouteOrView('product.edit'), ['code' => $this->product->code, 'lang' => request()->input('lang', config('app.locale'))]);
    }
    ...
    protected function saveProductVariation()
    {
        ...
        foreach ($arrayKeys as $i) {
            ...
            $product->setTranslation('description', request()->input('lang', config('app.locale')), $variationData['variable_description'][$i] ?? '');
    ...
    protected function loadProductVariations()
    {
        ...
        return $this->okResponse([
            'html' => view($this->getRouteOrView('admin.products.pieces.variations'), ['product' => $product, 'shippings' => $shippings, 'taxes' => $taxes, 'units' => $units])->render(),
    ...
    protected function productNameTranslate()
    {
        if (isset($this->request['name']) && ! empty($this->request['name'])) {
            $this->product->setTranslation('name', request()->input('lang', config('app.locale')), $this->request['name']);
            $this->product->save();
            unset($this->request['name']);
        }
    }

    protected function productOthersDataTranslate($data)
    {
        if (isset($data['description'])) {
            $this->product->setTranslation('description', request()->input('lang', config('app.locale')), $data['description']);
            unset($data['description']);
        }

        if (isset($data['summary'])) {
            $this->product->setTranslation('summary', request()->input('lang', config('app.locale')), $data['summary']);
            unset($data['summary']);
        }
