...
class ShippingProviderController extends Controller
{
    ...
    /**
     * Searches for shipping providers based on the provided search query.
     *
     * @param  Request  $request  The HTTP request object containing the search query.
     * @return JsonResponse A JSON response containing the search results.
     */
    public function searchShippingProviders(Request $request)
    {
        $data = ShippingProvider::where('status', 'Active')
            ->whereLike('name', $request->q)
            ->limit(20)
            ->get()
            ->map(function ($item) {
                return [
                    'id' => $item->slug,     // return slug instead of id
