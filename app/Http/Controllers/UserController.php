...
class UserController extends Controller
{
    ...
    public function create()
    {
        return view('admin.users.create', [
            'roles' => Role::getAll()->whereNotIn('slug', ['vendor-admin', 'guest', 'delivery-man', 'vendor-staff'])->whereNull('vendor_id')
        ]);
    }
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                ...
                if ($request->vendor_id) {
                    $id = (new User())->store($request->only('name', 'email', 'phone', 'password', 'status'));
                    $user = User::find($id);
                    $user->roles()->sync($request->vendor_role_id);
                    $user->vendors()->sync([$request->vendor_id]);
                    $data['status'] = 'success';
                    ...
                    if (isset($request->send_mail) && $request->status != 'Inactive' && ! empty($request['email'])) {
                        ...
                        User::whereEmailOrPhone($request['email'], $request['phone'])->first()?->notify($notification);
                    }
    ...
    public function edit($id = null)
    {
        ...
        if (! $data['user']->deliveryMan()->first()) {
            $role[] = 'delivery-man';
        }

        $data['roles'] = Role::getAll()->whereNotIn('slug', $role)->whereNull('vendor_id');
    ...
    public function updatePassword(Request $request, $id = null)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($response['status'] === true) {
                ...
                $request['phone'] =  $response['data']->phone ?? null;
                $request['raw_password'] = $request->password;
                ...
                if ((new User())->updateUser($request->only('password', 'updated_at'), $id)) {
                    if (isset($request->send_mail) && ! empty($request->send_mail)) {
                        User::whereEmailOrPhone($request->email, $request->phone)->first()?->notify(new UserPasswordSetNotification($request));
                    }
                    $data['status'] = 'success';
            ...
            if ($result['status'] === true) {
                ...
                try {
                    ...
                    if ($userStore) {
                        ...
                        $roleIds = $request->vendor_role_id ?? $request->role_ids;
    ...
    public function findUser(Request $request)
    {
        ...
        if ($request->from != 'pos') {
            $users = User::whereLike('name', $request->q)->limit(10)->get()->map(function ($user) {
                return [
                    'id' => 'user.' . $user->id,
                    'name' => $user->name,
                    'phone' => $user->phone,
        ...
        $customers = Customer::when($vendorId, function ($query, $vendorId) {
        ...
        if ($users->count() > 0 && $customers->count() > 0) {
            $combinedResults = $users->merge($customers);
        } elseif ($users->count() > 0) {
            $combinedResults = $users;
        } else {
            $combinedResults = $customers;
        }

        $formattedData = $combinedResults->map(function ($item) {
            $contactInfo = '';
            if (! empty($item['phone'])) {
    ...
    public function vendorList(Request $request)
    {
        ...
        if ($request->roleId != vendorStaffRole()) {
            return $this->errorResponse();
        }

        $data = $request->all();

        $data['vendorIds'] = \App\Models\Role::distinct()->pluck('vendor_id');

        if ($request->ajax() && $data['vendorIds']->count() > 0) {

            $html = view('admin.users.vendor_role', $data)->render();
     ...
     * @return \Illuminate\View\View
     */
    public function vendorRole(Request $request)
    {
        if ($request->ajax()) {
            $data = $request->all();
            $html = view('admin.users.vendor_specific_role', $data)->render();
    ...
    public function ledger(LedgerDataTable $dataTable, $id)
    {
        ...
        $data['orderTotal'] = Order::where('user_id', $userId)->sum('total');
        $data['transactionTotal'] = Order::where('user_id', $userId)->sum('paid');


        return $dataTable->render('admin.users.ledger', $data);
    }
}
