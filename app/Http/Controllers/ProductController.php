...
use App\Models\{Attribute, Brand, Category, Language, Product, ProductCategory, ProductMeta, Tag, Unit, Vendor};
...
class ProductController extends Controller
{
    ...
    public function createProduct(Request $request)
    {
        if ($request->isMethod('get')) {
            $data['brands'] = Brand::getAll()->where('status', 'Active')->where('is_global', 1);
            $data['vendors'] = Vendor::getAll()->where('status', 'Active');
            $data['categories'] = Category::getAll()->whereNull('parent_id')->where('status', 'Active');
            $data['attributeList'] = Attribute::getAll()->where('is_global', 1)->unique('name');
    ...
    public function edit(Request $request)
    {
        ...
        $data['attributeList'] = $product->vendor_id ? getVendorProductAttributes($product->vendor_id) : Attribute::getAll()->where('is_global', 1)->unique('name');
        $data['languages'] = Language::getAll()->where('status', 'Active');
        ...
        $data['brands'] = $product->vendor_id ? getVendorProductBrands($product->vendor_id) : Brand::getAll()->where('status', 'Active')->where('is_global', 1);
        if (isActive('Shipping')) {
            $data['shippings'] = ShippingClass::getAll();
        }
    ...
    public function deleteProduct(Request $request)
    {
        ...
        Product::deleteAssociatedRecords('id', [$product->id]);
    ...
    public function search(Request $request)
    {
        ...
        $query->whereIn('type', ['Simple', 'Variable', 'Simple Product', 'Variable Product', 'Variation']);
        $query->whereNull('parent_id');
        $query->isActiveVendor();
        $query->isAvailable();
        $query = $query->filter(ProductSearchFilter::class);
        ...
        foreach ($products as $product) {
            $variations = $product->getPurchaseableVariation();

            $parentAttributeOptions = $product->attributes;

            if (! empty($parentAttributeOptions) && is_array($parentAttributeOptions)) {
                $product->attributes = $parentAttributeOptions;
                $product->attributeValues = $product->getAttributeValues($parentAttributeOptions);

                $attributeKeys = array_keys($parentAttributeOptions);

                foreach ($variations as $variation) {
                    $variation->getAttributeMeta($attributeKeys);
                }
            }
    ...
    /**
     * load Vendor Brand Attribute
     *
     * @return render view
     */
    public function loadBrandAttribute($id)
    {
        $attribute = preference('vendor_attribute') ? getVendorProductAttributes($id) : false;

        $brand = preference('vendor_brand') ? getVendorProductBrands($id) : false;

        return response()->json(['status' => 'success', 'brand' => $brand, 'attribute' => $attribute]);
    }
}
