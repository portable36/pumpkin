...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $user           = User::select('id', 'name', 'email', 'status', 'phone', 'birthday', 'gender', 'address', 'created_at')
                            ->orderByRaw("email = 'walkingcustomer@gmail.com' DESC")
                            ->orderByDesc('id');
        ...
        if (! empty($roleId)) {
            ...
            if ($roleId == 2) {
                $user->whereHas('vendors', function ($q) {
                    $q->where('vendors.status', 'Active');
                });
            }
        }

        if (! isSuperAdmin() && auth()->user()->vendor()?->vendor_id) {
            $user->whereHas('associatedVendors', function ($q) {
                $q->where('id', auth()->user()->vendor()->vendor_id);
            });
        }
            ...
            if (is_int($keyword)) {
                ...
                $user->where(function ($query) use ($keyword) {
                    $query->where('name', 'LIKE', '%' . $keyword . '%')
                        ...
                        ->orWhereHas('vendors', function ($q) use ($keyword) {
        ...
        try {
            ...
            if (! empty($id)) {
                if (isset($request->send_mail) && validateEmail($request->email)) {
                    User::whereEmailOrPhone($request->email, $request->phone)->first()?->notify(new WelcomeUserNotification($request));
                }
                DB::commit();
    ...
    public function updatePassword(Request $request, $id)
    {
        ...
        if ($response['status'] === true) {
            ...
            $request['phone'] =  $response['data']->phone;
