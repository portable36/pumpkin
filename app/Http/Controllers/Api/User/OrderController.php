...
class OrderController extends Controller
{
    ...
    {
        // Check if url has pos then it work, otherwise ignore
        ...
        request()->merge(['custom_price' => $request->custom_price ?? 0]);
        ...
        if (is_array($cartData) && count($cartData) > 0) {
            ...
            $shippingCharge = $request->meta_order_type == 'pos' ? ($request->meta_shipping_charge ?? 0) : $totalShipping;
            ...
            $order['total'] = (Cart::totalPrice('selected') + $shippingCharge + $totalTax) - ($coupon + $otherDiscountAmount);
            $order['total_quantity'] = Cart::totalQuantity('selected');
            $order['paid'] = request()->custom_price ?? 0;
            $order['amount_received'] = request()->custom_price ?? 0;
                ...
                if (! empty($orderId)) {
                    ...
                    foreach ($cartData as $key => $cart) {
                        ...
                        $detailData[] = [
                            ...
                            'order_by' => $key,
                            'shipping_charge' => $request->meta_order_type == 'pos' ? 0 : $shipping,
                            'tax_charge' => $tax,
                    ...
                    if (isset($request->meta_order_type) && isset($request->payment_name) && $request->meta_order_type == 'pos') {
                        ...
                        if ($this->posOrderComplete($latestOrder, $request)) {
                            ...
                            do_action('after_vendor_create_order');
    ...
    public function posOrderComplete($order, $request)
    {
        ...
        if (isset(request()->custom_price) && request()->custom_price > 0 && request()->custom_price < $order->total) {
            $orderStatusInfo = OrderStatus::getAll()->where('slug', 'partial-payment')->first();
            $orderPaymentStatus = 'Partial';
            $order->total = $originalPrice;
            $order->amount_received = request()->custom_price;
            $paymentLogStatus = 'partial';
            $isPartialPayment = true;
        } elseif (isset(request()->custom_price) && request()->custom_price == 0) {
            $orderStatusInfo = OrderStatus::getAll()->where('slug', 'pending-payment')->first();
            $orderPaymentStatus = 'Unpaid';
            $order->total = $originalPrice;
            $order->amount_received = 0;
            $paymentLogStatus = 'unpaid';
        }
        ...
        try {
            ...
            $order->is_delivery = $isPartialPayment ? 0 : 1;
