...
use App\Models\{CustomField, File, RoleUser, PermissionRole, Unit};
...
{
    $key = config('cache.prefix') . '.' . 'preferences';

    if (! app()->bound($key)) {
        return is_null($field) ? collect([]) : $default;
    }
    ...
    return $preferences->get($field, $default);
}
...
function hasPermission($permission = null, $userId = null)
{
    ...
    return in_array($actionName, PermissionRole::permissionNamesByRoleID($roleID));
}
 ...
 * @return object
 */
if (! function_exists('moduleData')) {
    function moduleData(?string $name = null)
    {
        if (is_null($name)) {
...
if (! function_exists('techEncrypt')) {
    ...
    function techEncrypt($string)
    {
        ...
        // Replace "+" with "_plus_"
        $encrypted = str_replace('+', '_plus_', $encrypted);
...
if (! function_exists('techDecrypt')) {
    ...
    {
        // Replace "_plus_" with "+"
        $encryption = str_replace('_plus_', '+', $encryption);
...
if (! function_exists('coreStatusSlug')) {
    ...
    {
        return ['pending-payment', 'failed', 'processing', 'completed', 'on-hold', 'cancelled', 'refunded', 'partial-payment', 'assigned', 'pickup', 'delivered'];
    }
}
...
if (! function_exists('getProductExportColumn')) {
    ...
    {
        $data = [
            'ID', 'Name', 'Type', 'SKU', 'Parent', 'Published', 'Regular price', 'Sale price', 'Date sale price start',
            'Date sale price ends', 'Available from', 'Available to', 'Categories', 'Tags',
            'Backorders allowed?', 'Images', 'In stock?', 'Stock', 'Manage Stock', 'Stock threshold', 'Shipping class',
            'Tax class', 'Attributes',
        ];
        ...
        if (! isActive('SaaS')) {
            $data = array_merge($data, ['Description', 'Short description', 'Is featured?', 'Weight (lbs)', 'Height (in)', 'Length (in)', 'Width (in)', 'Button text', 'External URL', 'Grouped products', 'Upsells', 'Cross-sells', 'Allow customer reviews?', 'Purchase note', 'Download limit', 'Download expiry days', 'Sold individually?', 'Downloads']);
        }
...
if (! function_exists('checkIfUserIsStaff')) {
    ...
    {
        if (! $userId) {
            $userId = optional(auth()->user())->id ?? optional(auth()->guard('api')->user())->id;
        }
...
if (! function_exists('getStaffVendorUserId')) {
    ...
    function getStaffVendorUserId()
    {
        ...
        if ($user && $user->vendor()?->vendor_id) {
            ...
            return \App\Models\VendorUser::where('vendor_id', $user->vendor()?->vendor_id)->min('user_id');
        }
...
if (! function_exists('getVendorProductBrands')) {
    /**
     * Get all active brands. If vendor_brand preference is active,
     * it will return the brands which are assigned to the current vendor.
     *
     * @return \Illuminate\Database\Eloquent\Collection
     */
    function getVendorProductBrands($vendorId = null)
    {
        if (! preference('vendor_brand') || ! isActive('SaaS')) {
            return \App\Models\Brand::getAll()->where('is_global', 1)->where('status', 'Active');
        }
        ...
        return \App\Models\Brand::where('status', 'Active')
            ->whereHas('vendorBrand', function ($query) use ($vendorId) {
...
if (! function_exists('getVendorProductAttributes')) {
    /**
     * Get all active product attributes. If vendor_attribute preference is active,
     * it will return the attributes which are assigned to the current vendor.
     *
     * @return \Illuminate\Support\Collection
     */
    function getVendorProductAttributes($vendorId = null)
    {
        if (! preference('vendor_attribute') || ! isActive('SaaS')) {
            return \App\Models\Attribute::getAll()->where('is_global', 1)->where('status', 'Active')->unique('name');
        }
        ...
        return \App\Models\Attribute::where('status', 'Active')
            ->whereHas('vendorAttribute', function ($query) use ($vendorId) {
                $query->where('vendor_id', $vendorId);
            })
            ->get()
            ->unique('name');
...
if (! function_exists('isChatActive')) {
    /**
     * Check if the Chat is active basis on Saas & preference
     *
     * @return string
     */
    function isChatActive()
    {
        return ! isActive('SaaS') && isActive('Ticket')
            ? preference('chat', '')
            : '';
    }
...
if (! function_exists('defaultUnit')) {
    /**
     * Get Default Unit
     *
     * @return object
