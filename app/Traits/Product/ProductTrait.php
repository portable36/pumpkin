...
trait ProductTrait
{
    ...
    public function getDisplayPrice()
    {
        if ($this->isVariableProduct()) {
            ...
            foreach ($variations as $variation) {
                $hasSalePrice = ! is_null($variation->sale_price) && $variation->sale_price !== '';
                $isSaleActive = $hasSalePrice && (
                    (! $variation->sale_from && ! $variation->sale_to) ||
                    (! $variation->sale_from && $variation->sale_to && $variation->sale_to >= now()) ||
                    ($variation->sale_from && ! $variation->sale_to && $variation->sale_from <= now()) ||
                    ($variation->sale_from && $variation->sale_to && $variation->sale_from <= now() && $variation->sale_to >= now())
                );
                $price = $isSaleActive ? $variation->sale_price : $variation->regular_price;
            ...
            if (! $hasSale) {
                // Fallback to regular price range if no sale prices are active
                $minPrice = $maxPrice = null;
                foreach ($variations as $variation) {
                    $this->updateMixMax($minPrice, $maxPrice, $variation->regular_price);
                }
        ...
        $hasSalePrice = ! is_null($this->sale_price) && $this->sale_price !== '';
        $isSaleActive = $hasSalePrice && (
            (! $this->sale_from && ! $this->sale_to) ||
            (! $this->sale_from && $this->sale_to && $this->sale_to >= now()) ||
            ($this->sale_from && ! $this->sale_to && $this->sale_from <= now()) ||
            ($this->sale_from && $this->sale_to && $this->sale_from <= now() && $this->sale_to >= now())
        );
     ...
     * @return mixed
     */
    public function getFormattedDisplayPrice()
    {
