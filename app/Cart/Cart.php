...
class Cart
{
    ...
    public static function add($data = [])
    {
        ...
        if (! $cart) {
            ...
            if ($cart[$data['id']]['is_individual_sale'] == 0) {
                ...
                $cart[$data['id']]['price'] = $data['price'];
                $cart[$data['id']]['error_type'] = $data['error_type'] ?? null;
                $cart[$data['id']]['error_message'] = $data['error_message'] ?? null;
                self::save($cart);
            }
    ...
    {
        $identifier = self::getPosSessionId() ?? self::userId() ?? getUniqueAddress();

        return Cache::get(config('cache.prefix') . '.cart.' . $identifier);
    }
    ...
    {
        return ! empty(self::getPosSessionId() ?? self::userId())
            ? new CartCollection(Cache::get(config('cache.prefix') . '.cart.' . (self::getPosSessionId() ?? self::userId())))
            : new CartCollection(Cache::get(config('cache.prefix') . '.cart.' . getUniqueAddress()));
    }
    ...
    public static function destroy($index = null, $action = 'single')
    {
        if ($action == 'single') {
            ...
            ! empty(self::userId())
                ? Cache::forget(config('cache.prefix') . '.cart.' . (self::getPosSessionId() ?? self::userId()))
                : Cache::forget(config('cache.prefix') . '.cart.' . (self::getPosSessionId() ?? getUniqueAddress()));
            self::destroyCoupon();
            self::selectedDestroy();
        }
    ...
    {
        if (! empty(self::getPosSessionId() ?? self::userId())) {
            Cache::put(config('cache.prefix') . '.cart.' . (self::getPosSessionId() ?? self::userId()), $cart, 30 * 86400);
        } else {
    ...
    {
        if (! empty(self::getPosSessionId() ?? self::userId())) {
            Cache::put(config('cache.prefix') . '.selected.' . (self::getPosSessionId() ?? self::userId()), $data, 30 * 86400);
        } else {
    ...
    {
        return ! empty(self::getPosSessionId() ?? self::userId())
            ? Cache::get(config('cache.prefix') . '.selected.' . (self::getPosSessionId() ?? self::userId()))
            : Cache::get(config('cache.prefix') . '.selected.' . getUniqueAddress());
    }
    ...
    {
        if (! empty(self::getPosSessionId() ?? self::userId()) && ! empty(Cache::get(config('cache.prefix') . '.selected.' . (self::getPosSessionId() ?? self::userId())))) {
            Cache::forget(config('cache.prefix') . '.selected.' . (self::getPosSessionId() ?? self::userId()));
     ...
     * @return int|null
     */
    public static function getPosSessionId()
    {
        return Cookie::has('pos_terminal_session')
            ? optional(json_decode(Cookie::get('pos_terminal_session'), true))['sessionId'] ?? null
            : null;
    }
    ...
    {
        $userId = null;
        if (isset(request()->invoice_user_id)) {
            $userId = request()->invoice_user_id;
    ...
    public static function checkCartData()
    {
        ...
        foreach ($data as $key => $cartData) {
            $item = request()->channel === 'pos'
                ? Product::find($cartData['id'])
                : Product::whereKey($cartData['id'])->published()->first();
            ...
            if (isset($item->vendor) && $item->vendor->status != 'Active' && request()->channel != 'pos') {
                self::destroy($key);
                unset($data[$key]);

                continue;
            }
            if (empty($item) || $item->getStockStatus() == 'Out Of Stock' && request()->channel != 'pos') {
                self::destroy($key);
                unset($data[$key]);
