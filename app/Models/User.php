...
class User extends Authenticatable implements BatchDeletable
{
    ...
    /**
     * Relation with delivery man model
     */
    ...
    protected static function siteUpdateValidation($data, $id)
    {
        $validator = Validator::make($data, [
            ...
            'gender' => 'required|max:6|in:Male,Female,Other',
    ...
    protected static function updateProfileValidation($data, $id)
    {
        $validator = Validator::make($data, [
            'name'        => 'required|min:3|max:191',
            'email'       => ['required', 'max:191', 'unique:users,email,' . $id, new CheckValidEmail()],
            'phone'       => ['required', 'min:7', 'max:15', 'unique:users,phone,' . $id, new CheckValidPhone()],
    ...
    public function getData($token)
    {
        ...
        $user = null;
        if (! empty($reset)) {
            if (!empty($reset->email)) {
                $user = parent::where('email', $reset->email)->first();
            } elseif (!empty($reset->phone)) {
                $user = parent::where('phone', $reset->phone)->first();
            }
        }
     ...
     */
    public function scopeWhereEmailOrPhone($query, $email = null, $phone = null)
    {
