...
class Order extends Model
{
    ...
    /**
     * The order that belong to the delivery man.
     */
    public function deliveryMens(): BelongsToMany
    {
        return $this->belongsToMany(DeliveryMan::class, 'delivery_man_orders', 'order_id', 'delivery_man_id');
    }
    ...
    /**
     * Foreign key with customer model
     *
     * @return \Illuminate\Database\Eloquent\Relations\BelongsTo
     */
    ...
    public function orderCommission($orderDetailId, $statusId)
    {
        ...
        if (($finalOrderStatus) && $finalOrderStatus == $statusId && ! empty($orderCommission)) {
            ...
            if (isset($orderCommission->orderDetail?->vendor)) {
                if ($orderCommission->orderDetail?->order?->channel == 'pos') {
                    $orderCommission->orderDetail?->vendor?->users()->first()?->user?->wallet()?->decrementBalance($totalCommission);
                } else {
