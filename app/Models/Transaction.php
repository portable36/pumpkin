...
class Transaction extends Model
{
     ...
     */
    public function storeData($data = [], $user = null)
    {
        if (is_null($user)) {
            $user = auth()->user();
        }
        ...
        if ($data['transaction_type'] == 'Withdrawal' || $data['transaction_type'] == 'Delivery_withdrawal') {
            $walletBalance = $user->wallet($data['currency_id'])->balance;
        ...
        if (parent::insert($data)) {
            if ($withdrawal) {
                $user->wallet($data['currency_id'])->decrementBalance($data['amount']);
            }
            self::forgetCache();
    ...
    public function updateData($data, $id)
    {
        ...
        if ($transaction->update($data)) {
            ...
            if (($transaction['transaction_type'] == 'Withdrawal' && $data['status'] == 'Rejected') || ($transaction['transaction_type'] == 'Delivery_withdrawal' && $data['status'] == 'Rejected')) {
