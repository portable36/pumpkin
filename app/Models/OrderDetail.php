...
class OrderDetail extends Model
{
    ...
    /**
     * Product label wise shipping track info
     *
     * @return void
     */
    public function shippingTrack()
    {
        return $this->hasOne(OrderShippingTrack::class, 'order_id', 'order_id')
            ->where('product_id', $this->product_id)
            ->where('track_type', 'product');
    }
    ...
    public function checkStatus($statusId)
    {
        ...
        } elseif ($orderStatusInfo->payment_scenario != 'paid' && $this->is_stock_reduce == 1 && $this->order->channel != 'pos') {
