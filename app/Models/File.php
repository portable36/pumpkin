...
class File extends Model
{
                ...
                switch ($options['isUploaded']) {
                    case true:
                        ...
                        if (isFileExist($filePath . $filename)) {
                            $continue = \Storage::disk()->move($filePath . $filename, $file, $uploadPath . DIRECTORY_SEPARATOR . $filename);
                        }
                    ...
                    default:
                        ...
                        if (! empty($options['size'])) {
                            ...
                            $continue = \Storage::disk()->putFileAs($uploadPath, $file, $filePath . $filenameUpload);
                        }
    ...
    public function resizeImageThumbnail($uploadedFilePath, $uploadedFileName, $thumbnailPath, $oldFileName = null)
    {
        if (isset($oldFileName) && ! empty($oldFileName)) {
            if (isFileExist($thumbnailPath . DIRECTORY_SEPARATOR . $oldFileName)) {
                \Storage::disk()->delete(base_path() . DIRECTORY_SEPARATOR . $thumbnailPath . DIRECTORY_SEPARATOR . $oldFileName);
            }
        }
        ...
        foreach ($sizes as $name => $ratio) {
            try {
                // v3: Use ImageManager with driver
                $manager = new \Intervention\Image\ImageManager(
                    new \Intervention\Image\Drivers\Gd\Driver() // or use Imagick\Driver()
                );
                $baseUrl = rtrim(\Storage::disk()->url(''), '/');

                $imagePath = ltrim(str_replace($baseUrl, '', $imagePath), '/');

                // Read the file content first
                $fileContent = \Storage::disk()->get($imagePath);
                $img = $manager->read($fileContent);

                $thumbnailPath = createDirectory($this->thumbnailPath($name));

                foreach ($ratio as $key => $value) {
                    // v3: Clone image for each size to avoid modifying the same instance
                    $resizedImg = clone $img;

                    // v3: Use scale() or scaleDown() instead of resize with constraint
                    $resizedImg->scaleDown($key, $value);

                    // v3: Encode the image before saving
                    $encoded = $resizedImg->encode();

                    \Storage::disk()->put(
                        $thumbnailPath . DIRECTORY_SEPARATOR . $replacedFilename,
                        (string) $encoded
                    );
                }
            } catch (\Intervention\Image\Exceptions\DecoderException $e) {
                // v3: Exception namespace changed
    ...
    public function downloadAttachment($id)
    {
        ...
        if ($file) {
            ...
            $imagePublic = public_path('/uploads' . DIRECTORY_SEPARATOR . $file->file_name);
            if (! empty($file->file_name) && isFileExist($image)) {
                ...
                return Response::download(\Storage::disk()->url($image), $imageName, $headers);
            } elseif (! empty($file->file_name) && file_exists($imagePublic)) {
                switch (strtolower(pathinfo($file->file_name, PATHINFO_EXTENSION))) {
                    case 'pdf':
                        $mime = 'application/pdf';
                ...
                return Response::download($imagePublic, $imageName, $headers);
            } else {
