...
class Product extends Model implements BatchDeletable
{
    use hasFiles;
    use HasTranslations;
    use ModelTrait;
    ...
    {
        return $this->hasMany(StockManagement::class, 'product_id', 'id');
    }
     ...
     * @return void
     */
    public function decrementCategorySalesCount()
    {
        $category_id = $this->productCategory->category_id;
     ...
     * @return int|mixed
     */
    public function mainOrInventoryStock($stock = 0)
    {
        if (! isActive('Inventory') || $this->type === ProductType::$Variable) {
            return $stock;
        }

        $vendorId = $this->type === 'Variation'
            ? $this->parentDetail?->vendor_id
            : $this->vendor_id;

        return \Modules\Inventory\Entities\StockManagement::productStock($this->id, $vendorId);
    }
     ...
     * @return void
     */
    public static function checkProductSlugJson()
    {
        $products = Product::published()->get();

        foreach ($products as $product) {

            if (! is_null($product->getRawOriginal('slug')) &&  json_validate($product->getRawOriginal('slug'))) {
                $product->setTranslation('slug', config('app.locale'), $product->slug);
                $product->save();
            }
        }
     ...
     */
    public static function inStockProducts($query, $limit)
    {
        $inStockProducts = collect();
        $offset = 0;
        $batchSize = 20;

        while ($inStockProducts->count() < $limit) {
            ...
            $inStock = $batch->filter(function ($product) {
                return $product->getStockStatus() != 'Out Of Stock';
            });
            $inStockProducts = $inStockProducts->merge($inStock);
            $offset += $batchSize;
        }

        $inStockProducts = $inStockProducts->take($limit);

        return Product::whereIn('id', $inStockProducts->pluck('id')->toArray());
    }

    /**
     * Delete associated records
     */
    public static function deleteAssociatedRecords($column, $records): void
    {
        if ($column == 'id') {
            $products = Product::whereIn('parent_id', $records)->pluck('id')->toArray();
            $records = array_merge($records, $products);

            StockManagement::whereIn('product_id', $records)->delete();
        }
    }
    ...
    {
        $taxClass = TaxClass::firstWhere('slug', $this->tax_classes);

        if (! $taxClass) {
            return [];
        }

        return TaxRate::select('name', 'tax_rate', 'priority', 'compound', 'shipping')->where([
