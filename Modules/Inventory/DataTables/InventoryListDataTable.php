...
class InventoryListDataTable extends DataTable
{
   ...
   */
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->editColumn('unavailable', function ($stocks) {
                ...
                $str = '';
                if (auth()->user()?->hasPermission('Modules\Inventory\Http\Controllers\InventoryController@adjust')) {
                    $str = '<a title="' . __('Adjust') . '" href="' . route('inventory.adjust', ['product_id' => $stocks->product_id, 'location_id' => $stocks->location_id]) . '" class="action-icon adjust_page"><i class="fas fa-pen-square"></i></a>&nbsp;';
                }
    ...
    */
    ...
    {
        $stocks = StockManagement::with('product:id,name')->select('*', DB::raw('sum(quantity) as available'))
            ->whereIn('stock_managements.type', stockKeyword())
    ...
    */
    ...
    {
        return $this->builder()
            ->addColumn(['data' => 'product', 'name' => 'product.name', 'title' => __('Product'), 'className' => 'align-middle'])
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => __('Adjust'), 'width' => '12%',
                ...
                'visible' => auth()->user()?->hasAnyPermission(['Modules\Inventory\Http\Controllers\InventoryController@adjust']),
