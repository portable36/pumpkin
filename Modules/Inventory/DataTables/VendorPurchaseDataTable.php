...
class VendorPurchaseDataTable extends DataTable
{
   ...
   */
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->editColumn('reference', function ($purchases) {
                return '<a href="' . route('vendor.purchase.view', ['id' => $purchases->id]) . '">' . wrapIt($purchases->reference, 10, ['columns' => 4]) . '</a>';
            ...
            ->rawColumns(['reference', 'supplier', 'location', 'purchase_date', 'payment_status', 'action', 'total_amount'])
            ->make(true);
    }
    ...
    */
    ...
    {
        $purchases = Purchase::select('id', 'reference', 'supplier_id', 'location_id', 'vendor_id', 'currency_id', 'purchase_date', 'total_amount', 'status', 'payment_status')->where('vendor_id', auth()->user()->vendor()->vendor_id)->with('location', 'supplier', 'currency', 'vendor')->filter();
