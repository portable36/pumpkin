   ...
   */
        ...
        return datatables()
            ...
            ->editColumn('location', function ($logs) {
                if (! empty($logs->location_id) && $logs->location?->name) {
                    return '<a href="' . route('vendor.location.edit', ['id' => $logs->location_id]) . '">' . wrapIt($logs->location->name, 15, ['columns' => 7]) . '</a>';
                }
            })->editColumn('purchase', function ($logs) {
                if (! empty($logs->purchase_id) && $logs->purchase?->reference) {
                    return '<a href="' . route('vendor.purchase.edit', ['id' => $logs->purchase_id]) . '">' . wrapIt($logs->purchase->reference, 20, ['columns' => 7]) . '</a>';
                }
            })->editColumn('product', function ($logs) {
                if (! empty($logs->product_id) && $logs->product?->name) {
                    if ($logs->product->slug) {
                        return '<a href="' . route('vendor.product.edit', ['code' => $logs->product->code]) . '">' . wrapIt($logs->product->name, 15, ['columns' => 7]) . '</a>';
                    }

                    return wrapIt($logs->product->name, 15, ['columns' => 7]);
                } elseif (! empty($logs->purchase_id)) {
                    $productNames = $logs->purchase?->purchaseDetail
                        ->map(function ($detail) {
                            if ($detail->product) {
                                $name = $detail->product->trashed()
                                    ? "{$detail->product->name} (" . __('deleted') . ')'
                                    : $detail->product->name;
                    ...
                    return wrapIt($productNames, 15, ['columns' => 7]);
                }
            })->editColumn('supplier', function ($logs) {
                if (! empty($logs->supplier_id) && $logs->supplier?->name) {
                    return '<a href="' . route('vendor.supplier.edit', ['id' => $logs->supplier_id]) . '">' . wrapIt($logs->supplier->name, 20, ['columns' => 7]) . '</a>';
                }
            })->editColumn('currency', function ($logs) {
                return wrapIt($logs->currency?->name, 20, ['columns' => 6]);
            })->editColumn('order', function ($logs) {
                if (! empty($logs->order_id) && $logs->order?->reference) {
                    return '<a href="' . route('vendorOrder.view', ['id' => $logs->order_id]) . '">' . wrapIt($logs->order->reference, 20, ['columns' => 7]) . '</a>';
                }
            })->editColumn('transfer', function ($logs) {
                if (! empty($logs->transfer_id) && $logs->transfer?->reference) {
                    return '<a href="' . route('vendor.transfer.edit', ['id' => $logs->transfer_id]) . '">' . wrapIt($logs->transfer->reference, 20, ['columns' => 7]) . '</a>';
                }
            ...
            ->rawColumns(['location', 'purchase', 'product', 'supplier', 'currency', 'order', 'transfer', 'quantity', 'transaction_type', 'price', 'log_type', 'note', 'created_at'])
            ->make(true);
    }
    ...
    */
    public function query()
    {
        ...
        $logs = Log::select('*')->whereHas('product', function ($query) use ($vendorId) {
            $query->where('vendor_id', $vendorId);
        })->orWhereHas('purchase', function ($query) use ($vendorId) {
            $query->where('vendor_id', $vendorId);
        })->orWhereHas('location', function ($query) use ($vendorId) {
            $query->where('vendor_id', $vendorId);
        })
            ->orderBy('id', 'DESC')->filter();
