...
Route::group(['prefix' => 'admin/inventory', 'namespace' => 'Modules\Inventory\Http\Controllers', 'middleware' => ['auth', 'locale', 'permission', 'web']], function () {
    ...
    Route::get('/supplier/{id}/ledger', 'SupplierController@ledger')->name('supplier.ledger');
    Route::get('/supplier/{id}/payment', 'SupplierController@payment')->name('supplier.payment');
    Route::post('/supplier/{id}/payment', 'SupplierController@paymentStore')->name('supplier.paymentStore');
    ...
    Route::post('/find-vendor-default-location-with-ajax', 'PurchaseController@findVendorLocation')->name('purchase.findVendorLocation');
