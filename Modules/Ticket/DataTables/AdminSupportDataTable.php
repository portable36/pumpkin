    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($tickets) {
                $edit = '<a title="' . __('Edit :?', ['?' => __('Ticket')]) . '" href="' . route('admin.threadEdit', ['id' => $tickets->id]) . '" class="action-icon"><i class="feather icon-edit-1"></i></a>&nbsp';
                ...
                $str = '';
                if (auth()->user()?->hasPermission('Modules\Ticket\Http\Controllers\TicketController@edit')) {
                    $str .= $edit;
                }
                if (auth()->user()?->hasPermission('Modules\Ticket\Http\Controllers\TicketController@delete')) {
                    $str .= $delete;
                }
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn([
                ...
                'visible' => auth()->user()?->hasAnyPermission(['Modules\Ticket\Http\Controllers\TicketController@edit', 'Modules\Ticket\Http\Controllers\TicketController@delete']),
