    ...
    <div class="navbar-wrapper">
        ...
        <div class="navbar-content scroll-div">
            <ul class="nav pcoded-inner-navbar">
                ...
                @php
                    $menus = Modules\MenuBuilder\Http\Models\MenuItems::menus(5);
                @endphp
                @foreach ($menus as $menu)
                    <li data-username="form elements advance componant validation masking wizard picker select" class="nav-item {{ $menu->class }} @if($menu->isParent()) pcoded-hasmenu @endif {{ $menu->isLinkActive() ? 'pcoded-trigger active' : '' }}">
                        <a href='{{ $menu->isParent() ?  "javascript:" : $menu->url("carrier") }}' class="nav-link"><span class="pcoded-micon"><i class="{{ $menu->icon }} neg-transition-scale"></i></span><span class="pcoded-mtext">{{ $menu->label_name }}</span></a>
                        @if($menu->isParent())
                            <ul class="pcoded-submenu">
                                @foreach ($menu->child as $submenu)
                                    <li class="{{ $submenu->isLinkActive() ? 'active' : '' }} {{ $submenu->class }}">
                                        <a href="{{ $submenu->url('carrier') }}">{{ $submenu->label_name }}</a>
                                    </li>
                                @endforeach
                            </ul>
                        @endif
