    ...
    <div class="col-sm-12" id="withdrawal-edit-container">
        <div class="card">
            <div class="card-header">
                <h5>{{ __('Withdrawal') }} >> {{ __('Edit :x', ['x' => __('Withdrawal')]) }}</h5>
                <div class="card-header-right">
                    <div class="d-flex {{ languageDirection() == 'ltr' ? 'me-4' : 'ms-4' }} mt-2">
                        <h4 class="text-secondary {{ languageDirection() == 'ltr' ? 'me-1' : 'ms-1' }} font-18 font-bold">{{ __('Status') }}: </h4>
                        @php
                            $color = ['Pending' => 'text-warning', 'Accepted' => 'text-primary', 'Rejected' => 'text-red'];
                        @endphp
                        <h4 class="{{ $color[$withdrawal->status] }} {{ languageDirection() == 'ltr' ? 'ms-1' : 'me-1' }} font-18">{{ $withdrawal->status }}</h4>
                    </div>
                </div>
            </div>
                ...
                <div class="form-tabs">
                    ...
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                            <form action="{{ route('admin.delivery.withdrawal.update', ['id' => $withdrawal->id]) }}" method="post" class="form-horizontal">
                                @csrf
                                <div class="row">
                                    <div class="col-md-7 border col-12 p-3">
                                        ...
                                        @if ($withdrawal->transaction_type == 'Delivery_withdrawal')
                                            <div class="form-group row mt-25">
                                            ...
                                            <div class="col-sm-8">
                                                @if ($withdrawal->status == 'Rejected')
                                                    <div class="form-group row" id="divNote">
                                                        <div id='note_txt_1'>
                                                            <div>
                                                                <p class="font-12 bg-light-red text-white px-2 py-1 rounded {{ languageDirection() == 'ltr' ? 'ms-1' : 'me-1' }}">{{ __("Rejected status can't be changed") }}</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                @else
                                        ...
                                        <div class="col-sm-8 px-0">
                                            <a href="{{ route('admin.delivery.withdrawal.index') }}" class="btn all-cancel-btn custom-btn-cancel">{{ __('Cancel') }}</a>
                                            @if ($withdrawal->status != 'Rejected')
                                    ...
                                    <div class="col-md-5 col-12">
                                        <div class="border p-4 bg-light text-center">
                                            ...
                                            <div class="mt-2">
                                                <small class="text-muted">{{ __('Transaction Amount') }}</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
