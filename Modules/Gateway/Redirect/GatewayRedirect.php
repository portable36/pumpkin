...
class GatewayRedirect
{
     ...
     * @return redirect
     */
    public static function paymentRoute($order, $amount, $currencyCode, $code, $request, $payMentId = null, $paymentType = null, $origin = 'order')
    {
        if (is_null($payMentId)) {
            $log = PaymentLog::create([
                ...
                'code' => $origin == 'order' ? $order->id : $code,
                'status' => 'pending',
            ]);
        } else {
        ...
        GatewayHelper::setPaymentCode($origin == 'order' ? $order->id : $code);
