...
if (! function_exists('withOldQueryString')) {
    ...
    {
        return array_merge(request()->only(['to', 'payer', 'code', 'order_id']), $params);
    }
}
...
if (! function_exists('getIntegrityKey')) {
    ...
    {
        $query = array_merge(request()->only(['to', 'payer', 'code', 'order_id']), $override);
        $route = implode('', $query);
...
if (! function_exists('withOldQueryIntegrity')) {
    ...
    {
        return array_merge(request()->only(['to', 'payer', 'code', 'integrity', 'order_id', 'paymentType']), $params);
    }
}
