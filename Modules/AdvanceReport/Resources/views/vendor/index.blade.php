    ...
    <div class="report-header-section">
        <div class="report-header-content">
            ...
            <div class="report-header-right">
                <form action="{{ route('vendor.advance-reports') }}" method="get" class="report-search-form">
            ...
            <div class="report-active-filters">
                @if($search || !empty($categories))
                        ...
                        @foreach($categories as $catSlug)
                            @foreach($allReports as $catKey => $categoryData)
                                ...
                                <span class="report-filter-badge">
                                    ...
                                    <a href="{{ route('vendor.advance-reports', array_merge(['search' => $search], ['categories' => array_values(array_diff($categories, [$catSlug]))])) }}" class="badge-close" title="{{ __('Remove filter') }}">
                    ...
                    <span class="report-filter-badge">
                        ...
                        <a href="{{ route('vendor.advance-reports', ['categories' => $categories ?? []]) }}" class="badge-close" title="{{ __('Remove filter') }}">
                            <i class="fa fa-times"></i>
                        </a>
                    </span>
                    @endif
                    @if($search || !empty($categories))
                    <a href="{{ route('vendor.advance-reports') }}" class="report-clear-all">
    ...
    <div class="report-grid-section">
        <div class="report-grid">
            @forelse($reports as $catKey => $categoryData)
                ...
                <a href="{{ route('vendor.advance-reports.show', $report['slug']) }}" class="report-card">
