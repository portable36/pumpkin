...
class AddonManager
{
    ...
    public static function upload($addonZip)
    {
        ...
        return Addon::find($zipped_file_name);
    }
     ...
     * @return void
     */
    public function transferTranslations($addonName)
    {
        $sourceLangPath = module_path($addonName, 'Resources/lang');

        if (! File::exists($sourceLangPath)) {
            return;
        }

        $destinationLangPath = resource_path('lang');

        $sourceFiles = File::files($sourceLangPath);

        foreach ($sourceFiles as $sourceFile) {
            $fileName = $sourceFile->getFilename(); // Example: "bn.json"
            $sourceFilePath = $sourceFile->getPathname();
            $destinationFilePath = $destinationLangPath . '/' . $fileName;

            $sourceData = json_decode(File::get($sourceFilePath), true);

            if (File::exists($destinationFilePath)) {
                $destinationData = json_decode(File::get($destinationFilePath), true);
            } else {
                continue;
            }

            $mergeData = $destinationData + $sourceData;

            $shortName = explode('.', $fileName)[0];

            saveJSONFile($shortName, $mergeData);
            saveJSFile($shortName, $mergeData);
        }
    }
}
