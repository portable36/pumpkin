...
$("#loginForm").on("submit", function (event) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == 1) {
                $(".login-message").removeClass("bg-pinks-2 p-7p md:p-2.5 mb-3 md:mb-18p border border-reds-3 rounded");
...
function resendCode(parent) {
    $(parent).text('30').removeClass('underline cursor-pointer resend-code').css({'border': '2px solid #888', 'height':'30px', 'width':'30px', 'border-radius': '50%', 'display':'inline-block','margin':'auto', 'padding':'1px', 'text-align':'center'})
...
$(document).on('click', 'a.user-verification', function() {
    ...
    var email = '';
    $(this).closest('.signin-form-container').find('input[name="email"]').each(function() {
        if ($(this).val() != '' && email == '') {
            email = $(this).val();
        }
    })
