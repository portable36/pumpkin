...
if ($(".main-body .page-wrapper").find("#item-details-container").length) {
    ...
    $("#clear-variation").click(function () {
        ...
        $('.product-view-summary .main-summary').removeClass("display-none");
        $('.product-view-summary .variation').addClass("display-none");
        $('#b2b_table').html('');
    ...
    function attributePrice() {
        ...
        let flag = true;
        let cleaned = attributePriceWithId.replace(/[\r\n]+/g, '');
        parseAttributePriceWithId = JSON.parse(cleaned);
        $.each(parseAttributePriceWithId, function (i, v) {
            if (getPossbileAttributeWithPrice(v.attributeIds)) {
                ...
                $('.product-view-summary .main-summary').addClass("display-none");
                $('.product-view-summary .variation').removeClass("display-none").html(v.description);
                ...
                if (v.is_show_stock_status) {
                    ...
                    if (v.stock_status == 'In Stock') {
                        $('#stock_qty').html("<span class='text-green-1 capitalize leading-4 bg-green-2 px-4 py-2 text-sm roboto-medium font-medium rounded mr-2.5 rtl-direction-space-left'>" + jsLang('In Stock') + "</span>");
                        if (v.is_show_stock_quantity) {
                            var message = jsLang(":x items remaining");
                            message = message.replace(':x' , v.stock_quantity);
                            $('#stock_qty').append(message);
                        }
                    } else {
...
$(document).ready(function () {
    function loadProducts(elementId, endpoint) {
        ...
        if (element.length && element.attr("data-productId")) {  // Check if element exists and has data-productId
            $.ajax({
                type: "GET",
                url: SITE_URL + "/" + endpoint + "/" + element.attr("data-productId"),
                success: function (data) {
                    element.html(data);
                    loadLazyImages();
                },
