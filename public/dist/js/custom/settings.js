...
$('#customer_signup, #vendor_signup, #delivery_signup').on('change', function() {
    if ($(this).val() == "0") {
        $(this).attr('value', '1');
    } else {
        $(this).attr('value', '0');
...
function searchCurrency() {
    ...
    triggerSelect2(".select-currency");
}
...
if ($('.main-body .page-wrapper').find('#company-settings-container').length) {
    ...
    function getCity( siso, ciso) {
        ...
        var ajaxUrl = url + "/admin/countries/" + ciso + "/cities";
        
        if (siso !== undefined && siso !== '') {
            ajaxUrl = url + "/geo-locale/countries/" + ciso + "/states/" + siso + "/cities";
        }
...
if ($('.main-body .page-wrapper').find('#api_key_container').length) {
    $('.edit-token').on('click', function() {
        ...
        // Set form action URL
        $('#edit_token_Form').attr('action', url);

        // Set the application name value
        $('#edit_app_name').val(name);

        // Check if Select2 is initialized and destroy it if it is
        var $editStatus = $('#edit_status');
        if ($editStatus.data('select2') !== undefined) {
            $editStatus.select2('destroy');
        }

        // Update the selected option based on the status
        $editStatus.find('option').each(function() {
            $(this).prop('selected', $(this).text() === status);
        });
...
if ($('.main-body .page-wrapper').find('#unit_container').length) {
    $('.edit-unit').on('click', function() {
        ...
        // Set form action URL
        $('#edit_unit_form').attr('action', url);

        // Set the application name and abbr value
        $('#edit_name').val(name);
        $('#edit_abbr').val(abbr);

        function resetSelect2(id, value) {
            var $select = $('#' + id);
        ...
        resetSelect2('edit_status', status);
        resetSelect2('edit_default', dflt);
    });
}
