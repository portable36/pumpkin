...
if ($('.main-body .page-wrapper').find('#invoice-view-container').length) {
    ...
    initializeSelect2Ajax('.user-ajax', ADMIN_URL + '/find-users-with-ajax', jsLang("Search for users by name"));
}
...
if ($('.main-body .page-wrapper').find('#invoice-view-container').length || $('.main-body .page-wrapper').find('#vendor-order-view-container').length) {
    ...
    triggerSelect2('#search_products');
...
$('.select-provider').on('select2:open', function () {
...
if (typeof vendorUrl !== 'undefined') {
    initializeSelect2Ajax('.vendor-ajax', vendorUrl, jsLang("Search for vendor by name"));
}
