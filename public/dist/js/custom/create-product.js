...
$(function () {
    ...
    const toggleConditionalTabVisibility = (className) => {
        $(".conditional-dom").each(function () {
            ...
            if ($(this).hasClass('global-stock-status') && $('.c-click').is(':checked')) {
                $(this).addClass("d-none");
            }
        });
    ...
    const handleCreatedProduct = (response) => {
        ...
        if (response.url) {
            itemUrl = response.url+'?lang='+selectedLang;
    ...
    $(document).on("change", "#product_language_id", function () {
        var selectedLang = $(this).val();
        var actionUrl = $('#ajaxReloadForm').attr('action').replace(/\/action.*$/, '').split('?')[0];
        var newUrl = actionUrl + '?lang=' + selectedLang;
...
$(document).ready(function () {
    ...
    if (typeof brandUrl !== 'undefined') {
        initializeSelect2Ajax('.brand-ajax', brandUrl, jsLang("Search brand"), false);
    }
    ...
    if (typeof tagsAjaxSearch !== 'undefined') {
        initializeSelect2Ajax('.tag-ajax', tagsAjaxSearch, jsLang("Search tag"), true, true);
    }

    if (typeof itemsAjaxSearch !== 'undefined') {
        initializeSelect2Ajax('.select-ajax', itemsAjaxSearch, jsLang("Search for products by name or slug."));
    }
})

$(document).on('change', '.variation-manage-stock', function () {
    const stockId = $(this).attr('id');
    const stockStatus = $('.variation-stock-status[data-stock-id="' + stockId + '"]');
    
    $(this).is(':checked') ? stockStatus.addClass('d-none') : stockStatus.removeClass('d-none');
});
