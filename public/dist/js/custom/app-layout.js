...
$(".card-accordion").on('click', function() {
    $(this).closest('.card').find('.accordion-body').slideToggle(500);
    
    $(this).find('.drop-down-icon').toggleClass('rotate-180');
});
...
function initializeSelect2Ajax(selector, url, placeholderText, trigger = true, tag = false) {
    $(selector).select2({
        ajax: {
            url: url,
            dataType: "json",
            ...
            processResults: function (jsonResponse) {
                return {
    ...
    if (trigger) {
        triggerSelect2(selector);
    }
}

function triggerSelect2(selector) {
    $(selector).on('select2:open', function () {
        const $parent = $(this).parent();
        const $input = $parent.find('.select2-search__field');
        const input = $input.length ? $input : $('.select2-search__field');
        
        if (input.val().length === 0) {
            input.val('   '); // Trigger empty fetch
            input.trigger('input');
            input.val('');
        }
    });
