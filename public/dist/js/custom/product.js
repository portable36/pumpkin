...
if (
    ...
    $('#vendor_id').on('change', function () {
        ...
        $.ajax({
            url: SITE_URL + '/get-brand-attribute-by-vendor/' + vendorId,
            method: "get",
            success: function (response) {
                if (response.status == 'success') {

                    if (response.brand) {
                        var data = '<option value="">' + jsLang('Select an Option') + '</option>';
                        $.each(response.brand, function (key, value) {
                            data += '<option value="' + value.id + '">' + value.name + '</option>';
                        })     
                        const $select = $('.product_brand_id');
                    ...
                    if (response.attribute) {
                        var data = '<option value="">' + jsLang('Custom product attribute') + '</option>';
                        $.each(response.attribute, function (key, value) {
                            data += '<option value="' + value.id + '">' + value.name + '</option>';
                        })     
                        const $selectAttribute = $('.attribute-list');
        
                        $selectAttribute.empty().html(data);
                        if ($selectAttribute.hasClass('select2-hidden-accessible')) {
                            $selectAttribute.select2('destroy');
...
$(document).ready(function () {
    $(".c-click").click(function () {
        $(".s-hide").toggleClass('d-none', $(this).is(":checked"));
