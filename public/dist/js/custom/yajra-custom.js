...
function buildFiltersQuery() {
    ...
    let $activeGroup = $('.group-filter.active');
    if ($activeGroup.length && $activeGroup.data('filter-value') !== 'All') {
        urlQuery += $activeGroup.data('filter-key') + '=' + $activeGroup.data('filter-value') + '&';
    }
    // Get all filter selects/inputs
    $('.filter').each(function(){
        urlQuery += $(this).attr('name') + '=' + encodeURIComponent($(this).val()) + '&';
    });
    return urlQuery;
}

$(document).on('change', '.filter', function() {
    let urlQuery = buildFiltersQuery();
...
$(document).on('click', '.group-filter', function() {
    ...
    $(this).toggleClass('active').siblings('.group-filter').removeClass('active');
    // Always include filters state in query, not just group-filter
    let urlQuery = buildFiltersQuery();
    $('#dataTableBuilder').DataTable().ajax.url(urlQuery).load();
});
...
$('#dataTableSearch').on('input', function () {
    ...
    if (searchTerm.length >= searchMinLength || searchTerm.length == 0) { // Only search if length > 2
