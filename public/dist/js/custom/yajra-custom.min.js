'use strict';$("#dataTableBuilder_length").addClass("p-0"),$("#dataTableBuilder").removeAttr("style"),$(document).on("click",".filterbtn",function(){$(this).toggleClass("btn-primary active")});function buildFiltersQuery(){let a="?",b=$(".group-filter.active");return b.length&&"All"!==b.data("filter-value")&&(a+=b.data("filter-key")+"="+b.data("filter-value")+"&"),$(".filter").each(function(){a+=$(this).attr("name")+"="+encodeURIComponent($(this).val())+"&"}),a}$(document).on("change",".filter",function(){let a=buildFiltersQuery();$("#dataTableBuilder").DataTable().ajax.url(a).load()}),$(document).on("click",".group-filter",function(){if($(this).hasClass("active"))return!1;$(this).toggleClass("active").siblings(".group-filter").removeClass("active");let a=buildFiltersQuery();$("#dataTableBuilder").DataTable().ajax.url(a).load()});let debounceTimer;$("#dataTableSearch").on("input",function(){clearTimeout(debounceTimer);let a=$(this).val().trim();(a.length>=searchMinLength||0==a.length)&&(debounceTimer=setTimeout(()=>{let b=$("#dataTableBuilder").DataTable();b.search(a).draw()},searchDelay))});const characters="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";function generateString(a){let b="";const c=characters.length;for(let d=0;d<a;d++)b+=characters.charAt(Math.floor(Math.random()*c));return b}function headerCheckBox(){$(".need-batch-operation").find("table thead tr").prepend(`
