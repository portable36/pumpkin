...
if ($('.main-body .page-wrapper').find('#user-add-container').length || $('.main-body .page-wrapper').find('#user-edit-container').length) {
    ...
    $("#role_id").on('change', function() {
        if($('#role_id').val() == vendorStaffRole) {
            getVendorList();
        } else {
            $('.vendor').remove();
...
$(document).ready(function () {
    if ($('.main-body .page-wrapper').find('#user-edit-container').length) {
        ...
        if (typeof vendorStaffRole !== 'undefined' && $('#role_id').val() == vendorStaffRole) {
            getVendorList();
        }
    }
})
...
function getVendorList() 
{
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#role_id').closest('.form-group.row').after(data.response.records.data);
            $('#vendor_id').select2();
            $('#vendor_role_id').select2();
...
function getVendorRole() 
{
    $.ajax({
        ...
        success: function (data) {
            $('.vendor.role').remove();
            $('#vendor_id').closest('.form-group.row').after(data.response.records.data);
            $('#vendor_role_id').select2();
        },
        error: function (xhr, status, error) {
...
$(document).on('click', '.toggle-password', function () {
    ...
    if ($input.attr('type') === 'password') {
        ...
        $icon.removeClass('fa-eye').addClass('fa-eye-slash');   // show “eye-slash”
    } else {
        $input.prop('type', 'password');
        $icon.removeClass('fa-eye-slash').addClass('fa-eye');   // show normal “eye”
    }
});
